  ...
  <div className={`fixed w-full ${isWhite ? isScrolled ? "bg-white" : "bg-transparent" : "bg-dark"}  z-50 xl:z-40 transition-all ease-in-out-circ`}>
    <div className="container">
      <nav className={`flex flex-row justify-between items-center transition-all ease-in duration-200 z-50 delay-300 py-4 border-b border-b-gray ${isScrolled ? 'border-opacity-25' : 'border-opacity-0'}`}>
        <Link className='z-[60] cursor-pointer navLink' onClick={() => scrollToSection("home", 0)} offset={0}>
        ...
        <div className='transition-all ease-out-cubic duration-300 h-screen fixed top-0 left-0 w-full opacity-0 invisible xl:visible xl:relative xl:opacity-100 xl:h-auto flex flex-col xl:flex-row xl:justify-end justify-between pb-8 pt-32 xl:pt-3 xl:pb-3 px-8 xl:px-6 z-40 bg-dark xl:bg-transparent' id='navMenu'>
